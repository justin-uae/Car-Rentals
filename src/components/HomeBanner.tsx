...
export default function HomepageBanner() {
    ...
    useEffect(() => {
        if (selectedLocation !== '') {
            ...
            navigate(`/fleets?location=${encodeURIComponent(selectedLocation)}`);
        }
    ...
    return (
        <>
            <div className="relative mt-2">
                ...
                {/* Location Selector - Luxury styled */}
                        ...
                        <div className="relative" ref={dropdownRef}>
                            ...
                            >
                                ...
                                <div className="flex-1 text-left">
                                    <p className="text-[10px] sm:text-xs text-gray-500 font-medium uppercase tracking-wide">Pickup Location</p>
                                    <span className={`${selectedLocation ? "text-gray-900 font-bold" : "text-gray-400 font-medium"} text-sm sm:text-base`}>
                                        {selectedLocation || "Select Your Location"}
                                    </span>
                                </div>
                                <ChevronRight className={`w-4 h-4 sm:w-5 sm:h-5 text-red-600 transition-transform ${showLocationDropdown ? 'rotate-90' : ''}`} />
                            </button>
                            ...
                            {showLocationDropdown && (
                                <div className="absolute top-full mt-2 sm:mt-3 w-full bg-white rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-5 max-h-96 overflow-y-auto z-50 border-2 border-gray-100">
                                    <div className="mb-3 sm:mb-4">
                                        <h3 className="text-xs sm:text-sm font-bold text-gray-700 uppercase tracking-wide">Select Pickup Location</h3>
                                        <p className="text-[10px] sm:text-xs text-gray-500 mt-1">Choose where to collect your vehicle</p>
                                    </div>
                                    {loading ? (
                                        <div className="text-center py-8 sm:py-12 text-gray-500">
                                            ...
                                            <p className="text-xs sm:text-sm font-medium">Loading locations...</p>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-2 gap-2 sm:gap-3">
                                            {uniqueLocations?.map((city: any, index: number) => (
                                                ...
                                                >
                                                    <div className="w-full h-20 sm:h-24 rounded-md sm:rounded-lg overflow-hidden shadow-md">
                                                        <LazyLoadImage
                                                    ...
                                                    <div className="flex items-center gap-1 sm:gap-1.5 justify-center">
                                                        <MapPin className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-600 flex-shrink-0" />
                                                        <span className="text-xs sm:text-sm font-bold text-gray-800 group-hover:text-red-700 transition-colors truncate">{city?.location}</span>
                                                    </div>
                                                </button>
                                            ))}
                                        </div>
                ...
                {/* Best Cities to Visit Section - Luxury Theme */}
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between mb-8 sm:mb-10 md:mb-12">
                            <div>
                                ...
                                <p className="text-gray-600 text-xs sm:text-sm md:text-base font-medium">Explore premium car rentals across the UAE</p>
                            </div>
                        ...
                        {loading ? (
                            <div className="text-center py-12 sm:py-16 text-gray-500">
                                ...
                                <p className="font-semibold text-sm sm:text-base">Loading locations...</p>
                            </div>
                        ) : (
                            <div
                            ...
                            >
                                {bestCitiesToVisit.map((city: any, index: number) => (
                                    <button
                                        ...
                                        className="relative min-w-[240px] sm:min-w-[280px] md:min-w-[320px] lg:min-w-[380px] rounded-xl sm:rounded-2xl overflow-hidden shadow-xl group cursor-pointer h-64 sm:h-72 hover:shadow-2xl transition-all transform hover:scale-[1.02]"
                                    >
                                        <LazyLoadImage
                                            src={city?.images?.edges?.[0]?.node?.url || city?.image || 'https://via.placeholder.com/400x300'}
                                            alt={city.title}
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-r from-red-900/20 to-gray-900/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6">
                                            <h3 className="text-white text-xl sm:text-2xl md:text-3xl font-black mb-1 sm:mb-2 drop-shadow-lg">{city?.location}</h3>
                                            <div className="flex items-center gap-1.5 sm:gap-2 text-white/90 text-xs sm:text-sm font-semibold">
                                                <span>Rent in {city?.location}</span>
                                                <ChevronRight className="w-3 h-3 sm:w-4 sm:h-4 group-hover:translate-x-1 transition-transform" />
                                            </div>
                                        </div>
                                        {/* Corner accent */}
                                        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 w-10 h-10 sm:w-12 sm:h-12 border-t-2 sm:border-t-4 border-r-2 sm:border-r-4 border-red-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                    </button>
                                ))}
                            </div>
                        )}
